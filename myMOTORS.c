...
int const FREQ_REQ = 50000;
int motorpins[] = {20, 21, 22, 23};

int calcMOD()
{
	return (DEFAULT_SYSTEM_CLOCK / (128 * FREQ_REQ)) - 1; 
}

int setPwmVal(float percentage)
{
	return (calcMOD() + 1) * (percentage / 100);
}
...
void initMotors()
{
	...
	{
		PORTB->PCR[motorpins[i]] &= ~PORT_PCR_MUX_MASK;
		PORTB->PCR[motorpins[i]] |= PORT_PCR_MUX(3);
	}
	...
	//Sets the TPM1 to the Edge-aligned PWM High-true pulses (clear Output on match, set Output on reload) mode.
...
void move(direction_t dir, int power)
{
	switch(dir)
	{
		case FORWARD :	TPM1_C0V = TPM1_C1V = setPwmVal(power);
										TPM2_C0V = TPM2_C1V = 0;
										break;
		case REVERSE :	TPM1_C0V = TPM1_C1V = 0;
										TPM2_C0V = TPM2_C1V = setPwmVal(power);
										break;
